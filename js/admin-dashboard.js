...
// ===== Función para llenar select de usuarios automáticamente =====
function loadUsersFilter(movimientos) {
  const users = new Set();
  movimientos.forEach(m => { if (m.userEmail) users.add(m.userEmail); });
...
function aplicarFiltros(movimientos) {
  ...
  const selectedMap = filterMapSelect.value;
  ...
  return movimientos.filter(m => {
    ...
    if (selectedUser) match = match && m.userEmail === selectedUser;
    if (selectedMap) match = match && m.mapId === selectedMap;

    if (startDate) {
      const created = m.createdAt?.toDate ? m.createdAt.toDate() : null;
      match = match && created && created >= startDate;
    }
    ...
    if (endDate) {
      ...
      match = match && created && created <= endOfDay;
    }
...
applyFiltersButton.addEventListener('click', () => {
  let todosMov = [];
  Object.values(movimientosPorDia).forEach(arr => todosMov = todosMov.concat(arr));

  const filtrados = aplicarFiltros(todosMov);

  // Renderizar tabla filtrada
  tableMovBody.innerHTML = '';
  if (!filtrados.length) {
    tableMovBody.innerHTML = '<tr><td colspan="9">No hay movimientos que coincidan con los filtros.</td></tr>';
  } else {
    filtrados.forEach(d => {
      const tr = document.createElement('tr');
      tr.innerHTML = `
  ...
  updateMovCharts(filtrados);

  // Reasignar eventos eliminar
  tableMovBody.querySelectorAll('.btn-delete').forEach(btn => {
    btn.addEventListener('click', async () => {
      const id = btn.dataset.id;
      if (confirm('¿Seguro que quieres eliminar este movimiento?')) {
...
onSnapshot(qMov, snap => {
  ...
  loadUsersFilter(todosMovimientos);
});
...
const filterExpirationInput = document.getElementById('filterExpiration');

// Luego asignamos el evento
filterProductInput.addEventListener('input', filtrarInventario);
filterExpirationInput.addEventListener('change', filtrarInventario);

// Función de filtrado
function filtrarInventario() {
    const termino = filterProductInput.value.toLowerCase();
    const fechaLimite = filterExpirationInput.value ? new Date(filterExpirationInput.value) : null;

    tableInvBody.querySelectorAll('tr').forEach(tr => {
        const texto = tr.textContent.toLowerCase();
        let mostrar = texto.includes(termino);

        if (mostrar && fechaLimite) {
            const caducidadTexto = tr.children[2]?.textContent || '';
            const caducidadFecha = caducidadTexto ? new Date(caducidadTexto) : null;
            mostrar = caducidadFecha ? caducidadFecha <= fechaLimite : false;
        }
