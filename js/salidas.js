    ...
    async function registrarCargaCompleta() {
        ...
        try {
            ...
            for (const item of cargaProductos) {
                console.log('Item a procesar en batch:', item); // DEBUG: Inspeccionar el item de la carga
                ...
                console.log('Datos de la celda (antes de actualizar):', cellData); // DEBUG: Inspeccionar datos de la celda

                if (item.isProductArray) { // Si el producto está en un array dentro de la celda
                    let foundProductInCell = false;
                    const updatedProducts = cellData.products.map(prod => {
                        const currentProdId = prod.productId || prod.id; // Asume 'id' si no hay 'productId'
                        const currentProdName = prod.productName || prod.name;
                        const currentProdBrand = prod.productBrand || prod.brand;
                        const currentProdPresentation = prod.productPresentation || prod.presentacion || prod.presentation; // Más robusto

                        // DEBUG: Muestra el producto actual en la celda para compararlo
                        console.log('Buscando en celda. Item:', item.name, item.brand, item.presentation, item.productId, '| Prod en celda:', currentProdName, currentProdBrand, currentProdPresentation, currentProdId);

                        // Coincidencia: primero por productId, luego por nombre, marca y presentación
                        if ( (item.productId && currentProdId === item.productId) || 
                             (currentProdName === item.name && currentProdBrand === item.brand && currentProdPresentation === item.presentation) ) {
                            
                            foundProductInCell = true;
                            // Usar el nombre de campo de cantidad original
                            prod[item.originalQuantityFieldName] = (prod[item.originalQuantityFieldName] || 0) - item.quantity;
                            if (prod[item.originalQuantityFieldName] < 0) {
                                // Esto no debería pasar si las validaciones previas funcionan
                    ...
                    // Filtrar productos con cantidad <= 0 si es necesario o dejar que se muestren con 0
                    // Por ahora, solo actualizamos la cantidad
                    batch.update(cellRef, { products: updatedProducts });

                } else { // Si el producto es un campo directo del documento (Mapa 3)
                    // Usar el nombre de campo de cantidad original
                    ...
                    // Si la cantidad llega a 0, podrías querer eliminar otros campos del producto directamente
                    // Por simplicidad, solo actualizamos la cantidad
                }
            }

            // 2. Registrar la salida completa en historial_salidas
            const salidaData = {
                fecha: fechaSalida,
                destino: destino,
                observaciones: observacionesTextarea.value.trim(),
                productos: cargaProductos.map(item => ({ // Guardar una copia de los productos de la carga
            ...
            await addDoc(historialDocRef, salidaData); // Añadir como un nuevo documento en historial_salidas
            ...
            // Limpiar la interfaz después de un registro exitoso
            clearProductSelectionFields();
            cargaProductos = []; // Vaciar la carga temporal
            renderCargaTable(); // Renderizar la tabla vacía
            destinoInput.value = '';
            observacionesTextarea.value = '';
            
            // Recargar todos los productos para reflejar los nuevos stocks
            ...
            registrarCargaBtn.disabled = false; // Re-habilitar el botón
            renderCargaTable(); // Asegurar que el estado del botón se actualice si falla y la tabla no esté vacía
        }
    }
